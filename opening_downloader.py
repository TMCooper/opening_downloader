...
def main():
    ...
    try:
        ...
        opening = input("quelle est le nom de votre anime ? : ")

        if opening in ["q", "Q"]:
                print("Sortie du programme...")
                exit()

        choix_op = input("Si l'anime a plusieur opening quelle est celui que vous souhaité ? : ")
        if not choix_op or int(choix_op) <= 1 :
            choix_op = 1
        else :
            choix_op = choix_op
        ...
        url = f'https://www.youtube.com/results?search_query={op_convert}+opening+{choix_op}'
        driver.get(url)
